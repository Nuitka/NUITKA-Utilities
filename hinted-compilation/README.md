...
* Tell Nuitka which **standard plugins** should be enabled (generate all required ``--enable-plugin`` arguments)
* Tell Nuitka which modules / packages should be loaded (generate all required ``--recurse-to`` arguments)
* When parsing the ``--recurse-to`` modules, for each encountered ``import`` statement Nuitka will ask whether to follow it. Based on its recorded information, the hinting logic will provide the appropriate answers. This will lead to a rather fine-grained set of packages, sub-packages and modules included in the ``dist`` folder.
...
``nuitka-hints.py`` **invokes the Nuitka compiler** with a number of parameters which you can adapt to your environment.
...
Later, during compilation, it will be asked by Nuitka whether to include specific components and decide appropriately. You may see a number of ``keep`` and ``drop`` info messages reflecting these decisions.
...
* Nuitka's command line options are also fully supported. Include them before your script. So you can add parameters for icon inclusion etc.
...
Or even more options may be needed to reflect your compiler choice, console window suppression and what not.
